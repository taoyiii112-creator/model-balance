...
- 2026-08-09：桌面端 v1.0.0 已发布（GitHub Release，用户授权；zip + exe 双资产 + SHA256）——版本号 0.2.4 → 1.0.0，包含系统通知与消息中心（Windows Toast + 应用内消息中心）及消息中心显示修复；与手机端 v1.0.0 同期发布。
- 2026-08-09（修复）：消息中心打开时消息列表不显示——heads 字典漏了隐藏列 "id"，Treeview 初始化抛 KeyError；已补上并用真实 Tk 窗口验证渲染正常。
